...
const getNavGroups = (alertCount: number, orderCount: number, chatCount: number, showAdmin: boolean): NavGroup[] => {
  const groups: NavGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        ...(showAdmin ? [{ id: 'admin' as Tab, icon: Shield, label: 'Admin' }] : []),
      ],
    },
  ];
  return groups;
...
export default function Sidebar({ active, onChange, alertCount = 0, orderCount = 0, chatCount = 0, showAdmin = false, onSearch }: SidebarProps) {
  ...
  const navGroups = getNavGroups(alertCount, orderCount, chatCount, showAdmin);
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        transition={{ type: 'spring', stiffness: 300, damping: 30 }}
        className="hidden lg:flex fixed top-0 left-0 bottom-0 z-40 bg-sidebar-background border-r border-sidebar-border flex-col overflow-hidden"
      >
        {sidebarContent}
      </motion.aside>
    </>
